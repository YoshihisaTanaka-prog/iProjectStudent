...
//            教師用へのコピペ時注意＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊＊
        if l.student.userId == currentUserG.userId && !l.isFinished{
            let t = l.startTime
...
func judgeIsNeedToGoLecturePage(){
    ...
    let checkTime2 = c.date(byAdding: .minute, value: -1, to: now)!
    for lt in lectureTimeListG{
        if(lt.startTime <= checkTime1 && checkTime2 <= lt.endTime ){
//                タイマーを止める
            stopLectureTimer()
