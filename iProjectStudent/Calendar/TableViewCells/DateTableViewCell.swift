...
class DateTableViewCell: UITableViewCell, UITextFieldDelegate {
    ...
    func setDate(dates: [Date]){
        ...
        print(self.tag, isAbleToEdit!, dates[0], limitDate!)
    ...
    @objc func doneBtn(){
        ...
        case 1:
            if selectedDate < limitDate{
                dateTextField.text = limitDate.ymdJp + " の"
                let c = Calendar(identifier: .gregorian)
                date = c.date(from: DateComponents(year: limitDate.y, month: limitDate.m, day: limitDate.d))!
