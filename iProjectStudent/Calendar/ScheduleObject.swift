...
class Schedules: ScheduleMonthDelegate {
    ...
    func loadSchedule(date: Date, userIds: [String], _ vc: UIViewController){
        ...
        if userIds.count == 2{
            if userIds[1] != currentUserG.userId{
                let ui = userIds[0]
    ...
    func showTimeFrame(date: Date) -> [[TimeFrameUnit]] {
        ...
        var teacher = User(userId: "", isNeedParameter: false, viewController: vc)
        if teacherId != "" {
            let teachers = vc.mixFollowList()
            for t in teachers{
                if t.userId == teacherId{
                    teacher = t
                    for ytl in teacher.youbiTimeList[date.weekId]{
                        let list = ytl.split(separator: ":")
                        teacherTimeList.append(Int(list.first ?? "0") ?? 0)
                    }
                    break
                }
            }
        ...
        if(self.monthDic[ym]?.dateDic[d] == nil || self.monthDic[ym]!.dateDic[d]!.count == 0){
            for i in businessHoursG[date.weekId].first..<businessHoursG[date.weekId].last{
                ...
                tfus.append(TimeFrameUnit(time: i, isMyEvent: false, shiftTimes: teacherTimeList))
                tfus.append(TimeFrameUnit(time: i, isMyEvent: true, shiftTimes: studentTimeList))
                ret.append(tfus)
            }
            return ret
        } else{
...
//                self.dateDic.removeAll()
                ...
                for o in objects{
                    let schedule = Schedule(schedule: o)
        ...
        for (key, value) in self.dateDic{
            ...
            while i < length{
                if v[i].time == v[i+1].time {
                    ...
                    v[i].scheduleIds += v[i+1].scheduleIds
                    v.remove(at: i+1)
                    length -= 1
                }
                i += 1
