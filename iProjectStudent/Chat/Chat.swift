...
class ChatRoom{
    ...
    init(_ chatRoom: NCMBObject, _ isGroup: Bool){
        ...
        self.lastSentDate = chatRoom.object(forKey: "lastSentDate") as? Date ?? Date()
        self.name = ""
                    ...
                    if(isGroup){
                        for object in objects {
                            ...
                            if(user.objectId == NCMBUser.current()?.objectId){
                                self.isPermited = object.object(forKey: "isPermited") as! Bool
                                break
                            }
                        }
                    }
                        ...
                        for i in 0..<objects.count {
                            let user = objects[i].object(forKey: "user") as! NCMBUser
                            users.append(user)
                            if user.objectId == NCMBUser.current()!.objectId {
                                myIndex = i
                            }
                        ...
                        if(myIndex == 0){
                            self.isPermited = objects[0].object(forKey: "isPermited") as! Bool
                            self.name = users[1].object(forKey: "name") as? String ?? "No name"
                        }
                        else{
                            self.isPermited = objects[1].object(forKey: "isPermited") as! Bool
                            self.name = users[0].object(forKey: "name") as? String ?? "No name"
                        }
                    }
                }
            }
